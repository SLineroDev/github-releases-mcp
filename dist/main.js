...
import { z } from "zod";
import { getGitHubConfig, formatRelease, fetchAllReleases, fetchReleaseByVersion, fetchReleasesBetweenVersions, extractPackageName, fetchOrganizationPackages, } from "./utils.js";
...
server.tool("github_releases_list", "List all GitHub releases for a repository with rich formatting and comprehensive details. Returns releases in chronological order with emojis for better readability (🔖 for versions, 🗓️ for dates, 📝 for descriptions). Perfect for getting an overview of a project's release history, finding the latest version, or monitoring release frequency. Supports pagination and filtering of pre-releases.", releasesListSchema, async ({ owner, repo, limit, includePreReleases = false, package: packageName }, context) => {
    const config = getGitHubConfig();
    try {
...
server.tool("github_org_packages", "List all packages for a GitHub organization. Returns detailed information about each package including its name, type, visibility, and repository details. Perfect for discovering available packages and their metadata in an organization.", orgPackagesSchema, async ({ org, package_type, visibility }, context) => {
    const config = getGitHubConfig();
    try {
...
server.tool("github_repo_readme", "Fetch the README file for a GitHub repository and return its plain text content. Useful for discovering package names and documentation in monorepos.", repoReadmeSchema, async ({ owner, repo }, context) => {
    const config = getGitHubConfig();
